...
const CandidatesAnalytics = () => {
  ...
  const { data: appliedJobs = [] } = useQuery({
    ...
    queryFn: async () => {
      const res = await axiosInstance.get(
        `/jobs/applied-jobs/${currentCandidate?._id}`
      );
      return res.data;
    },
  });
  ...
  const { data: MyShortlistedJob } = useQuery({
    ...
    queryFn: async () => {
      const res = await axiosInstance.get(
        `/jobs/applied-jobs/shortlist/approved/${currentCandidate?.email}`
      );
      return res.data;
    },
  });
  ...
  return (
    <div>
      ...
      <div className="grid lg:grid-cols-4 md:grid-cols-2 gap-5">
        <DashboardCard
          logo={<IoBagRemoveOutline />}
          title={"Total Jobs"}
          quantity={jobs?.totalJobs}
        />
        <DashboardCard
          logo={<VscGitStashApply />}
          title={"My Applications"}
          quantity={appliedJobs?.length}
        />
        <DashboardCard
          logo={<SiImessage />}
          title={"Total Message"}
        ...
        />
        <DashboardCard
          logo={<MdPlaylistAddCheck />}
          title={"Shortlisted"}
          quantity={MyShortlistedJob?.length}
        />
      </div>
